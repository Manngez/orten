...
const fold=value=>normalize(value).normalize("NFKD").replace(/\p{M}/gu,"");
const titleScore=place=>(preferredCodes.has(place.featureCode)?1_000_000_000:0)+place.population;
const cleanAlias=value=>value?.normalize("NFKC").replace(/[\u0000-\u001f\u007f]/g,"").replace(/\s+/g," ").trim();
function parseCountry(text,limit,europeOnly=false){const byName=new Map();for(const line of text.split("\n")){if(!line)continue;const c=line.split("\t");if(c.length<15||c[6]!=="P"||!acceptedCodes.has(c[7]))continue;const name=cleanAlias(c[1]),lat=Number(c[4]),lng=Number(c[5]),population=Number(c[14])||0;if(!name||!Number.isFinite(lat)||!Number.isFinite(lng)||europeOnly&&lng>60||/^(unknown|unnamed|ohne namen)$/i.test(name))continue;const rawAliases=[c[2],...(c[3]?.split(",")??[])].map(cleanAlias).filter(alias=>alias&&alias.length>=2&&alias.length<=80&&!/^[-+]?\d+(?:[.,]\d+)?$/.test(alias)&&fold(alias)!==fold(name)),aliases=[...new Map(rawAliases.map(alias=>[normalize(alias),alias])).values()].slice(0,8),place={name,lat,lng,population,featureCode:c[7],aliases},key=normalize(name),previous=byName.get(key);if(!previous||titleScore(place)>titleScore(previous))byName.set(key,place)}const ranked=[...byName.values()].sort((a,b)=>titleScore(b)-titleScore(a)||a.name.localeCompare(b.name));return{places:ranked.slice(0,limit).sort((a,b)=>a.name.localeCompare(b.name)),aliasPlaces:ranked.filter(place=>preferredCodes.has(place.featureCode)||place.population>=5000).slice(0,40)}}
...
const generatedAliasFile=rows=>`import type { CityAlias } from "./cityAliases";\n\n/** GeoNames-alias för större europeiska orter. Genererad automatiskt. */\nexport const GENERATED_CITY_ALIASES:CityAlias[]=[\n${rows.map(row=>`  {country:${escapeText(row.country)},name:${escapeText(row.name)},aliases:${JSON.stringify(row.aliases)}},`).join("\n")}\n];\n`;
const work=mkdtempSync(join(tmpdir(),"orten-geonames-")),counts={},generatedAliases=[],seenAliases=new Set();
try{for(const country of countries){const zip=join(work,`${country.code}.zip`);execFileSync("curl",["--fail","--location","--silent","--show-error",`${SOURCE}/${country.code}.zip`,"--output",zip],{stdio:"inherit"});execFileSync("unzip",["-q","-o",zip,"-d",work],{stdio:"inherit"});const text=readFileSync(join(work,`${country.code}.txt`),"utf8"),{places,aliasPlaces}=parseCountry(text,country.limit,country.europeOnly);for(const place of aliasPlaces){const aliases=place.aliases.filter(alias=>{const key=`${country.key}:${fold(alias)}`;if(seenAliases.has(key))return false;seenAliases.add(key);return true});if(aliases.length)generatedAliases.push({country:country.key,name:place.name,aliases})}counts[country.key]=places.length;writeFileSync(new URL(country.file,DATA_DIR),cityArray(country.exportName,places,country.code),"utf8")}writeFileSync(new URL("cityCounts.json",DATA_DIR),`${JSON.stringify(counts,null,2)}\n`,"utf8");writeFileSync(new URL("generatedCityAliases.ts",DATA_DIR),generatedAliasFile(generatedAliases),"utf8");const aliasCount=generatedAliases.reduce((sum,row)=>sum+row.aliases.length,0);if(aliasCount<2000)throw new Error(`För få automatiska ortsalias: ${aliasCount}`);console.log("Ortdata uppdaterad:",counts);console.log(`Genererade ${aliasCount} alternativa namn för ${generatedAliases.length} orter.`)}finally{rmSync(work,{recursive:true,force:true})}
