...
const emptyState:GameState={phase:"setup",mode:"classic",country:"sweden",unlockedCountries:[],players:[],eliminated:[],eliminationOrder:[],currentPlayerIndex:0,placedCities:[],usedCityNames:new Set(),lines:[],lastElimination:null,crossingLines:null,winner:null,scores:[],duelBreakTarget:1,breaks:[]};
...
export function useGame(){
  ...
  const startGame=useCallback((players:string[],mode:GameMode,country:Country="sweden",duelBreakTarget=1)=>{setHistory([]);setState({...emptyState,phase:"playing",players,mode,country,eliminated:players.map(()=>false),scores:players.map(()=>0),breaks:players.map(()=>0),duelBreakTarget:Math.max(1,Math.min(5,duelBreakTarget))})},[]);
  const placeCity=useCallback(async(raw:string)=>{
    ...
    const point=project(city.lat,city.lng,cityCountry),playerIndex=state.currentPlayerIndex,previous=state.mode==="duel"?state.placedCities.filter(placed=>placed.playerIndex===playerIndex).at(-1):state.placedCities.at(-1),points=previous?Math.max(10,Math.round(distance(previous.point,point)*2)):50;
    ...
    if(state.mode==="duel"&&crossing){
      ...
      if(winner){const counts=state.players.map((_,i)=>placedCities.filter(p=>p.playerIndex===i).length);updateStatsAfterGame(state.players,winner,eliminationOrder,placedCities.length,state.mode,scores,counts);setState({...state,phase:"gameover",placedCities,lines,usedCityNames:used,unlockedCountries,eliminated,eliminationOrder,lastElimination:eliminationOrder.at(-1)!,crossingLines:crossing,winner,scores,breaks})}
      else setState({...state,placedCities,lines,usedCityNames:used,unlockedCountries,eliminated,eliminationOrder,currentPlayerIndex:winnerIndex,lastElimination:eliminationOrder.at(-1)!,crossingLines:crossing,scores,breaks});
      return{success:true,points};
    }
    if(active<=1){const winner=state.players[eliminated.findIndex(v=>!v)]||state.players[playerIndex],counts=state.players.map((_,i)=>placedCities.filter(p=>p.playerIndex===i).length);updateStatsAfterGame(state.players,winner,eliminationOrder,placedCities.length,state.mode,scores,counts);setState({...state,phase:"gameover",placedCities,lines,usedCityNames:used,unlockedCountries,eliminated,eliminationOrder,lastElimination:crossing?eliminationOrder.at(-1)!:null,crossingLines:crossing,winner,scores})}else setState({...state,placedCities,lines,usedCityNames:used,unlockedCountries,eliminated,eliminationOrder,currentPlayerIndex:nextActive(eliminated,playerIndex),lastElimination:crossing?eliminationOrder.at(-1)!:null,crossingLines:crossing,scores});
    return{success:true,points};
  },[state]);
  ...
  const resetGame=useCallback(()=>{setHistory([]);setState(emptyState)},[]),setRemoteState=useCallback((next:GameState)=>{setHistory([]);setState({...next,unlockedCountries:next.unlockedCountries??[],breaks:next.breaks??next.players.map(()=>0),duelBreakTarget:next.duelBreakTarget??1,usedCityNames:new Set(next.usedCityNames)})},[]),undoLastMove=useCallback(()=>{const prev=history.at(-1);if(prev){setState(prev);setHistory(h=>h.slice(0,-1))}},[history]),unlockCountry=useCallback((country:NordicCountry)=>setState(current=>current.phase!=="playing"||country===current.country||current.unlockedCountries.includes(country)?current:{...current,unlockedCountries:[...current.unlockedCountries,country]}),[]),unlockCountries=useCallback((countries:NordicCountry[])=>setState(current=>current.phase!=="playing"?current:{...current,unlockedCountries:[...new Set([...current.unlockedCountries,...countries.filter(country=>country!==current.country)])]}),[]);
