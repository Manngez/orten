...
import { FINLAND_MAIN_PATH,FINLAND_OUTLINE_PATH } from "../data/finlandOutline";
import { PLAYER_COLORS } from "./GameSetup";
export default function GameBoard({state}:{state:GameState}){
  const norway=state.country==="norway",viewBox=state.finlandUnlocked?"0 0 520 700":norway?NORWAY_VIEWBOX:SWEDEN_VIEWBOX,width=state.finlandUnlocked?520:320;
  return <div className={`board ${state.finlandUnlocked?"finland-board":""}`}><svg viewBox={viewBox} preserveAspectRatio="xMidYMid meet">
    <defs><filter id="lineGlow"><feGaussianBlur stdDeviation="2" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter><filter id="finlandGlow"><feGaussianBlur stdDeviation="5" result="glow"/><feMerge><feMergeNode in="glow"/><feMergeNode in="SourceGraphic"/></feMerge></filter><linearGradient id="land" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#153c39"/><stop offset="1" stopColor="#0b2429"/></linearGradient><pattern id="grid" width="26" height="26" patternUnits="userSpaceOnUse"><path d="M26 0H0V26" fill="none" stroke="#61b7b0" strokeOpacity=".08" strokeWidth=".6"/></pattern></defs>
    <rect width={width} height="700" fill="#071b24"/><rect width={width} height="700" fill="url(#grid)"/>
    ...
    {state.finlandUnlocked&&<><path d={FINLAND_OUTLINE_PATH} fill="#0b2b35" stroke="#27d9ff" strokeWidth="2.4" filter="url(#finlandGlow)" className="finland-draw"/><circle r="5" fill="#7eeeff" filter="url(#finlandGlow)" className="finland-marker"><animateMotion dur="10s" fill="freeze" path={FINLAND_MAIN_PATH}/></circle></>}
