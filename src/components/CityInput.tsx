...
export default function CityInput({usedCityNames,onPlaceCity,disabled,country,unlockedCountries}:{usedCityNames:Set<string>;onPlaceCity:(n:string)=>Promise<{success:boolean;message?:string}>;disabled:boolean;country:Country;unlockedCountries:NordicCountry[]}){
  ...
  useEffect(()=>{let current=true;const value=query.trim();setSuggestions([]);setLoading(false);if(!value)return()=>{current=false};const timer=window.setTimeout(()=>{if(!current)return;setLoading(true);void searchCities(value,activeCountries(country,unlockedCountries),usedCityNames,6).then(result=>{if(current)setSuggestions(result)}).catch(()=>{if(current)setSuggestions([])}).finally(()=>{if(current)setLoading(false)})},SEARCH_DELAY_MS);return()=>{current=false;window.clearTimeout(timer)}},[query,usedCityNames,country,unlockedCountries]);
