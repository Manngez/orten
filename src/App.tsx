...
export default function App(){
  ...
  return <main className="game-shell">
    ...
    {nordicMenu&&<div className="modal-backdrop nordic-secret"><section className="nordic-menu"><p>HEMLIG MENY</p><h2>Aktivera ett land</h2>{(["finland","norway","denmark","germany","netherlands","belgium","luxembourg","france","estonia","latvia","lithuania","poland","switzerland","austria","hungary","italy","spain"] as const).map(country=>{const meta=COUNTRY_META[country],active=country===state.country||state.unlockedCountries.includes(country);return <button key={country} disabled={active} onClick={()=>activateCountry(country)}><span>{meta.flag}</span><b>{meta.name}</b><small>{active?"Aktiverat":"Lås upp"}</small></button>})}<button className="text-button" onClick={()=>setNordicMenu(false)}>Stäng</button></section></div>}
