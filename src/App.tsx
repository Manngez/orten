...
function OnlineLobby({role,status,name,room,error,lobby,mode,duelBreakTarget,country,playerId,onName,onRoom,onMode,onDuelBreakTarget,onCountry,onCreate,onJoin,onRetry,onReady,onBack,onStart}:{role:OnlineRole;status:OnlineStatus;name:string;room:string;error:string;lobby:LobbyPlayer[];mode:GameMode;duelBreakTarget:number;country:Country;playerId:string;onName:(v:string)=>void;onRoom:(v:string)=>void;onMode:(v:GameMode)=>void;onDuelBreakTarget:(v:number)=>void;onCountry:(v:Country)=>void;onCreate:()=>void;onJoin:()=>void;onRetry:()=>void;onReady:()=>void;onBack:()=>void;onStart:()=>void}){
  ...
  if(role==="offline")return <main className="online-shell"><section className="online-card"><div className="brand"><span className="brand-mark">O</span><span>ORTEN <b>ONLINE</b></span></div><p className="eyebrow">Spela på flera enheter</p><h1>Skapa eller anslut</h1><label>DITT NAMN<input value={name} onChange={e=>onName(e.target.value.slice(0,18))} placeholder="Exempel: Anna"/></label><label>RUMSKOD<input value={room} onChange={e=>onRoom(e.target.value.slice(0,24))} placeholder="Exempel: fredag"/></label><div className="online-actions"><button onClick={onCreate}>Skapa rum</button><button onClick={onJoin}>Gå med</button></div><button className="text-button" onClick={onBack}>← Tillbaka</button></section></main>;
